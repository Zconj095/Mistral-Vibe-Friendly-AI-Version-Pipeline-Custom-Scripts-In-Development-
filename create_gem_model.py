...
class CreateGemModelArgs(BaseModel):
    name: str = Field(description="Name of the custom GEM model profile.")
    system_prompt: str | None = Field(
        default=None,
        description="System prompt text for the custom GEM model.",
    )
    ...
    base_model: str | None = Field(
        default=None,
        description="Model alias to use (defaults to mistral-ollama; gpt-oss is also supported).",
    )
...
class CreateGemModelResult(BaseModel):
    name: str
    agent_path: str
    prompt_path: str
    base_model: str
...
class CreateGemModelConfig(BaseToolConfig):
    default_base_model: str = "mistral-ollama"
    max_name_length: int = 64
    max_prompt_bytes: int = 200_000
    max_instructions_bytes: int = 50_000


class CreateGemModelState(BaseToolState):
    pass


class CreateGemModel(
    BaseTool[
        CreateGemModelArgs,
        CreateGemModelResult,
        CreateGemModelConfig,
        CreateGemModelState,
    ],
    ToolUIData[CreateGemModelArgs, CreateGemModelResult],
):
    description: ClassVar[str] = (
        "Create a custom GEM model profile (agent + system prompt) in ~/.vibe."
    )

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, CreateGemModelArgs):
            return ToolCallDisplay(summary="create_gem_model")

        return ToolCallDisplay(
            summary=f"Creating GEM profile: {event.args.name}",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if isinstance(event.result, CreateGemModelResult):
            return ToolResultDisplay(
                success=True,
                message=f"Created GEM profile: {event.result.name}",
                details={
    ...
    def get_status_text(cls) -> str:
        return "Creating GEM profile"

    @final
    async def run(self, args: CreateGemModelArgs) -> CreateGemModelResult:
    ...
    def _build_prompt_text(self, name: str, prompt: str | None) -> str:
        if prompt is None or not prompt.strip():
            prompt = (
                f"You are a custom GEM model named '{name}'.\n"
