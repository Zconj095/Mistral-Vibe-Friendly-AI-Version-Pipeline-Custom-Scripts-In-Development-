...
class ProcessCodeDocumentsRecursiveConfig(BaseToolConfig):
    ...
    default_code_extensions: list[str] = Field(
        ...
        description="Default code extensions.",
    )
...
class ProcessCodeDocumentsRecursiveState(BaseToolState):
    pass


class ProcessCodeDocumentsRecursiveArgs(BaseModel):
    paths: list[str] = Field(description="Root directories or files to scan.")
    code_extension: str | None = Field(
        default=None, description="Single code extension to match (e.g. .py)."
    )
    code_extensions: list[str] | None = Field(
        default=None, description="Code extensions to match (e.g. ['.py', '.ts'])."
    )
    document_extension: str | None = Field(
        default=None, description="Single document extension to match (e.g. .md)."
    )
    document_extensions: list[str] | None = Field(
        default=None, description="Document extensions to match (e.g. ['.md', '.txt'])."
    )
...
class MixedDocumentEntry(BaseModel):
    path: str
    kind: str
    language: str | None
    bytes_total: int
...
class ProcessCodeDocumentsRecursiveResult(BaseModel):
    mode: str
    documents: list[MixedDocumentEntry]
...
class ProcessCodeDocumentsRecursive(
    BaseTool[
        ProcessCodeDocumentsRecursiveArgs,
        ProcessCodeDocumentsRecursiveResult,
        ProcessCodeDocumentsRecursiveConfig,
        ProcessCodeDocumentsRecursiveState,
    ],
    ToolUIData[ProcessCodeDocumentsRecursiveArgs, ProcessCodeDocumentsRecursiveResult],
):
    description: ClassVar[str] = (
        "Recursively find code and document files by extension and read or chunk them."
    )

    async def run(
        self, args: ProcessCodeDocumentsRecursiveArgs
    ) -> ProcessCodeDocumentsRecursiveResult:
        if not args.paths:
        ...
        code_exts = self._resolve_extensions(
            "code",
            args.code_extension,
            args.code_extensions,
            self.config.default_code_extensions,
        )
        doc_exts = self._resolve_extensions(
            "document",
            args.document_extension,
            args.document_extensions,
            self.config.default_document_extensions,
        )

        include_globs = self._resolve_include_globs(args, code_exts, doc_exts)
        ...
        truncated = False
        document_count = 0
        code_set = set(code_exts)
        doc_set = set(doc_exts)
        ...
        for file_path in files:
            ...
            try:
                ...
                kind = self._resolve_kind(ext, code_set, doc_set)
                if kind is None:
                    raise ToolError(
                        f"{file_path} has unsupported extension {ext or '(none)'}."
                    )
                ...
                language = self._resolve_language(file_path) if kind == "code" else None
                ...
                if kind == "code":
                    ...
                    file_chunks, file_truncated = self._build_code_chunks(
                        lines,
                        boundaries,
                        depths,
                        unit,
                    ...
                    self._finalize_code_chunks(file_chunks)
                    ...
                    if len(source_chunks) > remaining:
                        ...
                        self._finalize_document_chunks(source_chunks)
                        truncated = True
        ...
        return ProcessCodeDocumentsRecursiveResult(
            mode=process_mode,
            documents=documents,
            chunks=chunks,
    ...
    def _resolve_include_globs(
        self,
        args: ProcessCodeDocumentsRecursiveArgs,
        code_exts: list[str],
        doc_exts: list[str],
    ) -> list[str]:
        ...
        combined = sorted(set(code_exts + doc_exts))
        if not combined:
            raise ToolError("Provide code/document extensions or include_globs.")

        return [f"**/*{ext}" for ext in combined]
    ...
    def _resolve_extensions(
        ...
        if single and multiple:
            raise ToolError(
                f"Provide {label}_extension or {label}_extensions, not both."
            )
        ...
        return self._normalize_extensions(ext_list)
    ...
    def _resolve_kind(
        self, ext: str, code_set: set[str], doc_set: set[str]
    ) -> str | None:
        if ext in code_set:
            return "code"
        if ext in doc_set:
    ...
    def _finalize_code_chunks(self, chunks: list[CodeChunkData]) -> None:
        total = len(chunks)
        for chunk in chunks:
    ...
    def _chunk_content(
        ...
        return self._finalize_document_chunks(chunks)
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, ProcessCodeDocumentsRecursiveArgs):
            return ToolCallDisplay(summary="process_code_documents_recursive")

        summary = f"process_code_documents_recursive: {len(event.args.paths)} path(s)"
        return ToolCallDisplay(
            summary=summary,
            details={
                "paths": event.args.paths,
                "code_extension": event.args.code_extension,
                "code_extensions": event.args.code_extensions,
                "document_extension": event.args.document_extension,
                "document_extensions": event.args.document_extensions,
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, ProcessCodeDocumentsRecursiveResult):
    ...
    def get_status_text(cls) -> str:
        return "Processing code and documents recursively"
