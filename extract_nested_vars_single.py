...
class ExtractNestedVarsSingleConfig(BaseToolConfig):
...
class ExtractNestedVarsSingleState(BaseToolState):
    pass


class ExtractNestedVarsSingleArgs(BaseModel):
...
class ExtractNestedVarsSingleResult(BaseModel):
    symbols: list[NestedSymbol]
    count: int
    errors: list[str]


class ExtractNestedVarsSingle(
    BaseTool[
        ExtractNestedVarsSingleArgs,
        ExtractNestedVarsSingleResult,
        ExtractNestedVarsSingleConfig,
        ExtractNestedVarsSingleState,
    ],
    ToolUIData[ExtractNestedVarsSingleArgs, ExtractNestedVarsSingleResult],
):
    description: ClassVar[str] = (
        "Extract nested variables across classes/structs/functions from a single snippet."
    )

    async def run(self, args: ExtractNestedVarsSingleArgs) -> ExtractNestedVarsSingleResult:
        snippet_id, language, content = self._load_snippet(args)
        ...
        return ExtractNestedVarsSingleResult(
            symbols=symbols,
            count=len(symbols),
            errors=errors,
        )

    def _load_snippet(self, args: ExtractNestedVarsSingleArgs) -> tuple[str, str | None, str]:
        ...
        snippet_id = args.id or (args.path or "snippet")
        if args.content is not None:
            return snippet_id, language, args.content

        path = self._resolve_path(args.path or "")
        snippet_id = args.id or str(path)
    ...
    def _parse_generic(
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            if attr_match and include_fields:
                ...
                if owner:
                    add_symbol(
                        attr_match.group(2), "field", idx, attr_match.start(2), owner=owner
                    )
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, ExtractNestedVarsSingleArgs):
            return ToolCallDisplay(summary="extract_nested_vars_single")

        return ToolCallDisplay(
            summary="extract_nested_vars_single",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, ExtractNestedVarsSingleResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )

        message = f"Extracted {event.result.count} symbols"
    ...
    def get_status_text(cls) -> str:
        return "Extracting nested variables (single snippet)"
